...
def wu_sum(np.ndarray [DTYPE_t, ndim=2] array, double p1_0, double p1_1, double p2_0, double p2_1, bint count=False, int width=0, bint debug=False):
    ...
    cdef unsigned int proper_x0 = int(np.ceil(x0))
    cdef unsigned int proper_x1 = int(np.floor(x1))

    cdef double weight_first = proper_x0 - x0
    cdef double weight_last  = x1 - proper_x1
    cdef unsigned int proper_y0 = 0
    cdef unsigned int proper_y1 = 0
    cdef double fst_offset = 0
    cdef unsigned int fst_y = 0
